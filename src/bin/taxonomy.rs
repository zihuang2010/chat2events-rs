...
//! cargo run --release --bin taxonomy -- . summaries 2026-08-01 2026-08-31 | head -200
//!
//! # 2. ★ 人手写 taxonomy_v2.toml：逐个 [[types]] 填 type_id / parent_name / name / description
//!
//! # 3. 试打，看未分类率和空类数，不行就回第 2 步改
//! cargo run --release --bin taxonomy -- . review taxonomy_v2.toml 2026-08-01 2026-08-31
//!
//! # 4. 定稿转 SQL，人工执行
//! cargo run --release --bin taxonomy -- . emit-sql taxonomy_v2.toml > taxonomy_v2.sql
//! mysql ... < taxonomy_v2.sql
//!
//! # 5. 把 classify::CURRENT_VERSION 改成 "v2"，重新编译部署，再跑 recompute
//! ```
//!
//! 第 3 步不可省 —— 词表定下后不再漂移，而 `event_type` 一旦逐日漂移，
//! 就没有报表能建在这个维度上（`CONTEXT.md`）。
use chat2events_rs::{Result, boot::Boot, process::taxonomy};
use std::path::Path;
...
async fn main() -> Result<()> {
    ...
    // ⚠️ **`Boot::load` 必须在 emit-sql 分支之后** —— 见上面那段注释。
    ...
    match cmd {
        ...
        "review" => {
            ...
            let p = taxonomy::review_draft(&b.config, &llm, &d, &sums, out).await?;
