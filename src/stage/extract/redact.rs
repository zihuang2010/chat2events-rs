...
use crate::stage::ingest::Message;
use regex::Regex;
...
pub(super) fn body(m: &Message) -> String {
    ...
    // ⚠️ 第三段语义上是**零宽前瞻** `(?=\s*/|$)`，而 Rust 的 regex 没有前瞻 ——
    // 所以把分隔符吃进第 3 组再原样吐回去（`${3}`，见 [`FIELD_TO`]）。等价，但别写成贪婪。
