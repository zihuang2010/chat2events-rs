...
/// `"王鸿江：\n<被引用的原文>"\n------\n<回复正文>`
///
/// 1850 条样本里 20 条这种结构，而 [`render`] 已经把那条 `replyTo` 渲染成 `↩回复 #N`
/// 了 —— 所以引用块是同一信息的第二份拷贝，且是**带姓名**的那份。
...
/// 不支持后顾断言，断言由 [`phone_mask`] 手写，见那里（那两个断言是承重的）。
static PHONE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"1[3-9]\d{9}(?:-\d{4})?").unwrap());
...
/// 拦住 `summary` 里的它们 · [`SYSTEM`] 教模型「这不是原文、不承载关联信息」。
///
...
pub(super) static PLACEHOLDER: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(&format!("{MASK_PHONE}|{MASK_FIELD}|{MASK_AT}")).unwrap()
});
...
fn phone_spans(s: &str) -> Vec<(usize, usize)> {
    ...
    let mut pos = 0usize;
    while pos < s.len() {
        let Some(m) = PHONE.find(&s[pos..]) else { break };
        let start = pos + m.start();
        let mut end = pos + m.end();
        // 带后缀却后接数字 -> 退回不带后缀（Python 引擎会为了满足 (?!\d) 而回溯）
        if end - start > 11 && end < bytes.len() && bytes[end].is_ascii_digit() {
            end = start + 11;
        }
