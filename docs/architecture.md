...
`source_messages` 为 `NULL` **有两个来源**：加这一列之前抽取的历史行 · 过了保留期被
`store::prune_source_messages` 清空（每轮跑批自动执行，与镜像区同一个保留期，是承重不变量 1
的具名例外 1-A）。下钻对两者都返回 410 —— 页面语义相同，排查时按 `occurred_on` 落在
保留期边界哪一侧区分。
...
扫描和 2 次名册 HTTP，还把首屏推后一个 RTT。2026-09-19 那个路由已删，窗口的默认与夹取
一直都在 `params::Period::bounds` 里。 —— ⚠️ **群与客服名单跟着查询窗口走**
...
（⚠️ **这一串是估算，2026-09-19 已用 dev 库实测校正**：真正错得多的是**群数** ——
实际 307 不是 1000；而「15.7 事件/群日」接近实测的活跃群日速率 13.71，本身不离谱。
默认七天窗口实测约 **1.3 万行**，不是 11 万。下面那个 181 同样跟着缩水，
按实测要 **460 群左右**才会撞上 `max_rows`。见 `docs/deploy.md`「实测基线」）
