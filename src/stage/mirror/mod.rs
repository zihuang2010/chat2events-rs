//! ① 摄取 · 镜像 —— 把本地 raw 区同步成 OSS 月文件的**字节级镜像**。
//!
...
//! 跟 `ingest` 的 [`crate::stage::ingest::IngestError`] 一个规矩。
//!
//! **路径布局不在这里** —— 写文件问 [`crate::stage::ingest::room_path`] 要路径，跟读的是同一个
...
//!   tests.rs     跨文件共享的测试素材（MonthFile 的 fixture）
//! ```
