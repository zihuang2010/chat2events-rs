...
| 展示列 | 写进事实表、但**不参与任何指标**的列（当前只有 `source_messages`，事件的未脱敏原文快照）。有**独立于事实的保留期** —— 与 raw 镜像同一个，过期后置空，**冻结区的行也清**。那是承重不变量 1 的具名例外 1-A，判据与已知边界见 `docs/invariants.md`，别在这里复述 |
| 已知成功群日 | 三条判据同时成立的一个「群 × 日」：抽取状态为 `ok` · 有事实完成凭据 · 该群日所属窗口内没有晚于该凭据的抽取失败。它是承重不变量 5 的**分母边界** —— 聚合指标只算这些群日上的事件，其余群日的旧事实仍可查阅（核实用）但不计入。**有前后端两个实现**，改一处必须看另一处 |
...
取数走 HTTP：`<DOWNLOAD_BASE_URL>/<ndjson_object_key>`，`Range` 增量。
...
summary                 事件摘要
last_msg_role           末条来源消息发送方角色 —— asker_role 的镜像，取末条而非首条。
                        EXTERNAL=商家说完没人接 / INTERNAL=客服收的尾。
                        NULL=加这一列之前抽取的历史行，不是某一边（冻结区不可回填）
followup_wait_max_sec   后续轮次最长等待秒数 —— 首响之后每次 EXTERNAL→INTERNAL 取最大。
                        工作时段口径 [08:30, 21:00)，周末与节假日不扣（没有工作日历）。
                        末尾没人接的那一段不计（那是 last_msg_role 的活）。
                        与首响时效同一个工作时段口径；区别在可改性，见 src/worktime.rs。
                        0=确实没有后续轮次 / NULL=没算过，两者绝不混
```

**标注列**：新事实先保存，首次打标随后完成；冻结区已有标签只因词表升版重打。

```
event_type              事件类型（一个事件一个类）；未完成时为 NULL
...
**「不含人名」这一条已经撤销。** 原始理由是「模型只看到匿名标签，天然保证」—— 而 `body` 决定不掩正文里的姓名，那个前提就没了。实测 476 个 summary 里 **18 条（3.8%）含师傅姓名**（「商家要求订单转给杨师傅，平台已安排」）。
...
**其余三条都有 validator**（Rust 版是 `extract::model::validate`），不靠 prompt 里叮嘱。曾经只校验了长度那一条，另两条纯靠 prompt —— 实测 282 个 summary 确实一条没漏，但那是「没有护栏」不是「已经安全」。校验失败把报错原文回灌进下一轮 prompt。

**但三条的处置不一样**，分界是「这是不是 PII」：

| | 含 ID（订单号 / 手机号） | 含脱敏占位符 | 超过 100 字 |
|---|---|---|---|
| 是 PII 吗 | 是 | **否**，是 PII 被抹掉留下的洞 | 否 |
| 处置 | 硬失败，整批作废 | **就地抹除** | 重问一次后放行 |

ID 那条硬失败，因为 `summary` 归**事实列**：冻结区不可写，且 `sha256(summary)` 是 classify 的缓存键 —— **PII 一旦进去就是永久的，缓存还会把它焊死**。所以挡在 validator，**不做落库前 scrub**（那会改内容、让缓存键漂掉）。

占位符不同：正文脱敏后模型看到的是 `<手机号>` / `<略>`，抄进 summary 就成了「商家发来`<手机号>`」—— **不是泄漏**，只是脱敏留下的记号不是内容。它在 validator 里**就地抹掉**（三个记号是常量，抹除是确定性的）—— 这不是「落库前 scrub」，`summary` 从一开始就是抹完的值，缓存键就是它的 sha256，没有东西可漂。走抹除不走重问，是因为 prompt 的 summary 规则里早已有明令和改写范例，实测仍然照犯：事件本身就是「改地址」「换电话」时，唯一的锚点已经被抹成 `<略>`，模型只能照抄。

长度那条同理不是数据问题 —— 列宽是 `VARCHAR(200)`，101 字装得下。重问一次给模型自己压缩的机会，仍不改就放行；硬闸挪到列宽上（`assemble` 的 `SUMMARY_COLUMN`）。

⚠️ 三条曾经同等处罚（都是硬失败），实测一轮打挂 11 个群，其中 9 个只因 ×1 条 summary 带了个脱敏记号就整日 0 事件落库。
...
| **v1+** | `recompute . v1 <since> <until>` 重打标 + 重算指标 | 以上全部 **+ 分类明细** |
