...
//!
//! 端口（换数据源那天：新写一个文件实现这两个函数，其余阶段一行不动）：
//!
//! ```text
...
//! ```
//!
//! ⚠️ **端口上曾经还有第三个 `read_by_ids`，已删。** 它唯一的读取点是 webUI 下钻，
//! 而下钻改读 `b_merchant_group_event.source_messages`（抽取时写入的渲染快照）之后，
//! 只读工作台一个文件都不读 —— raw 区的保留期从此只约束跑批的输入。
//!
...
//!   read.rs    DuckDB 连接 · SQL · 上游字段语义 · 五道守卫 · read_room / read_synced_room
//! ```
//!
//! 端口那两个函数里 `list_rooms` 住在 `layout.rs` —— 它只遍历目录、不碰 DuckDB。
