...
///
/// ⚠️ **它不再决定 webUI 下钻能看多久以前的原文** —— 下钻改读
/// `b_merchant_group_event.source_messages`（抽取时写入的渲染快照），
/// 只读工作台一个文件都不读。这里删掉的只是**跑批重跑时还能不能读到原文**。
///
/// ⚠️ 粒度是**整个月目录**：一个群一个月一个文件，删不了半个。所以 M 月的原文
/// 在 M+2 月 4 号那轮消失，月末的消息只活 33 天、月初的活 63 天。
///
