...
或者漏跑一天，消息就落在预期之外的分片里，**静默漏掉**。要做到精确就得让 `mirror`
...
`ingest.months(days) -> ["202608", "202609"]`，`mirror` 查索引表、`list_rooms` 遍历目录、
`read_room` 选文件三处共用。
...
`ingest/tests.rs` 里的 `月份守卫的已知盲区` 那条测试把它写下来了。
...
* 今天 `mirror/` 和 `llm.rs` 用的是同一个 TLS 栈，两个端点实测都通，无需任何改动。
...
要人工删文件才恢复。不修，因为 `have` 只由 `mirror` 自己写，而它只写通过校验的完整
