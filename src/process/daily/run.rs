...
use super::{
    labeling::{ClassifyTally, ClassifyTask, classify_room, run_classification},
    tally::{Outcome, Tally},
};
use crate::{
    Result,
    ...
    join,
    llm::Llm,
    stage::classify::{CURRENT_VERSION, Classifier},
    stage::extract::{self, Event, LiveModel, SegmentModel, Usage},
...
/// 别让补跑在日志上跟日常跑批长得一模一样。见 `src/bin/backfill.rs`。
///
...
pub async fn run_span(
    ...
    // 原文快照（`b_merchant_group_event.source_messages`）跟镜像区**同一个保留期**。
    // 那是未脱敏的客户正文，而看板无登录 —— 两个副本的留存期分家是顺手改出来的，
    // 不是决定。清的是展示列不是事实列，不碰冻结（见 `store::prune_source_messages`）。
    //
    // 清理失败**不掀翻整轮**：跟上面的 `prune` 一个待遇，它是维护动作，
    // 失败一次下一轮再来，不该让一整轮已经烧完 token 的事实白费。
    if let Some(before) = w
        .since()
        .checked_sub_months(chrono::Months::new(config.ingest.raw_retention_months - 1))
        .and_then(|d| d.with_day(1))
    {
        // 下界 = 刚滑出保留期的那一个月（理由见 `prune_source_messages`）。
        let since = before
            .checked_sub_months(chrono::Months::new(1))
            .unwrap_or(before);
        match store::prune_source_messages(pool, since, before).await {
            ...
            |corp, room| {
                ...
                async move {
                    let outcome = run_room(
                        &root,
                        &pool,
                        // `&*` 不是噪声：`run_room` 收的是端口 `&impl SegmentModel`，
                        // 泛型参数不做 deref 强转，`&Arc<LiveModel>` 不会自己变成 `&LiveModel`。
                        &*model,
                        run_date,
                        &corp,
                        &room,
    ...
    // 打标跟抽取用**同一个** deadline：过点不开新批次，已在飞的等完。见 `run_classification`。
    let classification = run_classification(rx, concurrency, deadline, |task| {
    ...
    // 打标那一侧到点没轮到的群 —— **`stage` 是 `classify` 不是 `extract`**：
    // 它们的事实是好的，只是标签没算完。用 `extract` 会让这批完好的事实掉出
    // `KNOWN_OK_DAYS` 的分母（那条只看 `stage = 'extract'`），把一次**打标**的预算不足
    // 伪装成**抽取**不可信。标签保持 NULL，等 `src/bin/recover.rs` 人工补齐。
    for (corp, room) in &classified.skipped {
        if let Err(e) = record_failure(
            pool,
            run_date,
            corp,
    ...
    // ⚠️ **`extracted?` 必须排在上面那个记账循环之后。** 走 `IngestError::Upstream`
    // 那条路时（整轮错误），预算到点被砍的那几百个群本来一行账都没有 —— 库里对它们
    // 就是整行缺失，而 `Tally` 只活在这一轮的内存里，几个月后没有任何东西解释得了
    // 报表上的洞。先把 `skipped` 记完再抛，抛出去整轮照样非零退出。
    extracted?;

    // 收尾：产能账 · 空轮告警 · 退出码。**纯函数，`daily/tests.rs` 直接调得到** ——
    // 此前这 99 行是 `run_span` 里唯一没测试的一段，而它恰好压着三条退出码规则。
    let outcome = finish(
        &t,
        &classified,
        rooms.len(),
    ...
    // 索引统计跟着这一轮的删重写一起刷新（见 `store::refresh_statistics`）。
    // ⚠️ **放在收尾日志之后、几条提前 return 之前** —— 有群失败时统计照样该刷新，
    //    那一轮的删重写已经发生了。它自己不改退出码。
    //    `finish` 返回 `Result` 而不是就地 `return`，这个顺序才留得住。
    store::refresh_statistics(pool).await;
    outcome
}

/// 按本轮实测外推「全部群跑完要几小时」。`t.ok == 0` 时返回 `None` —— 一个群都没跑完
/// 就没有分子，给 0 等于拿 0 冒充「没算出来」（承重不变量 4）。
///
/// ⚠️ **分母是 `t.ok`，不是 `rooms`。** 没轮到的、拉取失败的、空窗口的都没发过请求，
/// 掺进去会把每群成本摊薄 —— 而错法是静默的：外推值只会偏小一点，没有任何东西会报错。
/// 单独一个函数就是为了让这条有地方可测。
pub(super) fn projected_extract_hours(
    t: &Tally,
    rooms: usize,
    usage: Usage,
    concurrency: usize,
) -> Option<f64> {
    (t.ok > 0).then(|| usage.secs / t.ok as f64 * rooms as f64 / concurrency as f64 / 3600.0)
}

/// 一轮的收尾 —— 产能账 · 空轮告警 · 退出码，**三样都是纯计算**。
///
/// 抽出来的理由是它此前是 `run_span` 里唯一没测试的一段，而 `run_rooms` /
/// `rotate_daily` / `stride` 三个兄弟的 doc 都写着「分出来只为让那条性质离线可测」——
/// 于是「循环可测、结论不可测」。下面 `rooms` 为空那条注释记的就是它漏掉的第一个 bug。
///
/// **不碰 pool、不做 IO。** `store::refresh_statistics` 留在调用方，
/// 因为它必须落在收尾日志之后、提前 return 之前，而这里返回 `Result` 不就地 return。
#[allow(clippy::too_many_arguments)]
pub(super) fn finish(
    t: &Tally,
    classified: &ClassifyTally,
    rooms: usize,
    config: &Config,
    w: &Window,
    usage: Usage,
    ...
    //
    // ⚠️ **分母只能用 `extracted`（真的跑完的群），不能用 `rooms`**：没轮到的、
    ...
    let projected_extract_hours =
        projected_extract_hours(t, rooms, usage, config.ingest.room_concurrency);
    tracing::info!(
        rooms = rooms,
        extracted = t.ok,
    ...
    // 在日志上长得一模一样，而库里是空的。
    // 「一行都没写」有两种，**诊断完全不同，所以分开报**：
    //
    //   * 一个群都没有 —— 索引表一行都没查到；
    //   * 有群、但每个群窗口内都没消息 —— 上游停更 / 窗口和数据错开。
    //
    // ⚠️ 第一种此前**不响**：条件是 `t.empty > 0 && t.empty == rooms`，而
    // `rooms` 为空时 `t.empty` 也是 0，`0 > 0` 为假；后面两条 `return Err` 同样
    // 不成立（没轮到的、失败的、没同步的都是零），于是整轮以退出码 0 空转过去，
    // 监控看到的是成功。它能空的路径包括上游改掉 `file_month` 的格式
    // （`ingest::months` 生成、`mirror::index` 拿去 bind 上游列）—— 匹配不上不报错，
    // 只返回零行。「上游字段**名**只许出现在 index.rs」守住了名字，守不住这个格式。
    //
    // 仍然只是 warn 不改退出码：新部署第一天、新企业接入都会合法地空一轮，
    // 报错会把它们一起拦下。要更严就在这里 `return Err`，那是个产品决定。
    if rooms == 0 {
        tracing::warn!(
            since = %w.since(),
            until = %w.until(),
            "索引表没给出任何可拉的群，一行都没写 —— \
             先查 b_wecom_group_message_month_file 的筛选条件\
             （file_month 格式 / file_status / is_deleted / 窗口是否落在有数据的月份）"
        );
    } else if t.empty == rooms {
        tracing::warn!(
            rooms = rooms,
            since = %w.since(),
            until = %w.until(),
...
///
/// `model` 收的是**端口**不是适配器。此前这里写死 `&LiveModel`，唯一的理由是
/// `run_span` 收尾要 `model.usage()` 算产能外推 —— 但那是 `run_span` 手里那个
/// `Arc<LiveModel>` 的事，跟本函数无关。写死的后果是 ③ 那条**真接缝**
/// （两个适配器：`LiveModel` / `BisectStub`）在唯一的生产调用点上被绕过：
/// `architecture.md` 承诺「换模型只改一个 `SegmentModel` 适配器」，而实际还得改编排。
#[allow(clippy::too_many_arguments)]
pub(super) async fn run_room(
    raw_root: &std::path::Path,
    pool: &MySqlPool,
    model: &(impl SegmentModel + Sync),
    run_date: NaiveDate,
    corp: &str,
    room: &str,
    w: &Window,
    ...
    let conv = match conv {
        ...
        Err(e) => {
            ...
            if let Err(write_error) =
                record_failure(pool, run_date, corp, room, w, "extract", &reason).await
            {
    ...
    // 客服日消息量同属消息级 —— **不传 `status`，也不该传**：它是「谁说了多少」，
    // 抽取失败照样是已知的，那正是它用来对冲「只看处理量」的前提。
    let agent_msg = metrics::agent_msg_rows(corp, room, &agent_msg_counts);
...
///
/// `"extract"` 是**共用**的 stage：拉取失败、没轮到、抽取失败三者对事实新鲜度的
/// 后果相同 ——「这个群的窗口本轮没被重新处理，事实可能落后于上游」。影响面由
/// `run_failure` 上的 `window_since/until` 夹住（`store::record_failure` 里有全文），
/// 不再靠 stage 区分。要按原因分开查是可诊断性需求，那时再给枚举加值。
/// ⚠️ **`stage` 必须由调用方给。** 此前这里硬编码 `"extract"`，而打标侧也要记账 ——
/// 拿 `extract` 记打标的失败会让一批完好的事实掉出聚合分母（`KNOWN_OK_DAYS` 只看
/// `stage = 'extract'`），指标只是变小，不会报错。
