...
//!   pipeline.rs  调用链：分段 → 段调用 → 自适应二分 · 切点选择（cut / segments）
...
// 这里只把名字接出去 —— 调用方（`daily` / `store` / 各入口）仍然写
// `extract::LiveModel` / `extract::Event` / `extract::extract`，接口一字未动。
pub use model::{LiveModel, SegError, SegmentModel, Usage};
pub use pipeline::{extract, preview};
pub use types::{Event, EventDraft, SourceMessage};
