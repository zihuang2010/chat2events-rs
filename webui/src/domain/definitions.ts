...
export const METRIC = {
  ...
  rooms:
    "当前范围内至少有一起事件的群，按 roomid 去重。抽取失败那天库里没有事件行，所以失败的群可能不出现在这里，以完整性提示为准。",
  agentsInvolved:
  ...
  overdue:
    "分子是首响超过阈值的商家发起事件，无响应计入分子（T+1 跑批下无响应必然已超过任何阈值）。分母是商家发起事件数。自然时间口径，跨夜与周末照常计时。",
  backlog:
  ...
  coverage:
    "抽取失败的「群 × 日」在 metric_daily 上事件级列是 NULL 不是 0，在 agent_metric_daily 上是整行缺失。本页所有事件级数字都不含它们，是偏小，不是真实下降。",
  msgCount: "metric_daily 的消息级列，不依赖抽取，失败的群照样有。它是消息条数，不是事件数。",
...
export const DATA_GAPS = [
  ...
  },
  {
    title: "客服姓名与群名",
    detail:
      "领域里只有 16 位 easyUserId 和 officialRoomId，样本中没有任何字段带姓名。界面上凡是显示别名的地方都带「待补」标记。",
    needs:
