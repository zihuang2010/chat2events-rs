...
//! B 路径（本地 embedding + HDBSCAN + LLM 命名，`examples/taxonomy_embed.rs`）
...
//! ```text
//! taxonomy/
//!   run.rs     取 summary（`store` 的只读口子）· 拿草稿试打并落 review.md
//!   draft.rs   草稿文件（TOML）· 校验 · 转 INSERT SQL
//!   review.rs  试打的实现（命中数 · 代表样例 · 未分类率）
//! ```
...
pub use review::{Report, render, review};
pub use run::{review_draft, summaries};
