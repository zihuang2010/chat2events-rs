...
**落地记录（2026-09-01，`mirror/` 搬完）：我们一个 feature 都没显式选，继承的是
...
## 补充决策（2026-09-01）：本地保留 2 个月，下钻超出保留期时回 OSS 取

原文只说了「本地是月文件镜像」，**没说留多久 —— 于是一天也没删过**。
按 `config.toml` 自己的注释算：每天 600 MB，一年 216 GB，**永不回落**。
这是这条流程上唯一真实的欠账，也是当初「不做按天分片」这个决定留下的账。

### 8. 保留最近 2 个月，删除起点锚在**窗口**上

`ingest::prune(raw_root, w, retention)`，每轮**紧跟 `mirror::sync`** 执行 ——
不放收尾：收尾那儿有好几条提前 `return Err`，只要有群失败就轮不到清理，
而磁盘偏偏会在最该清的那些天继续涨。

保留起点 = `w.since()` 往前推 `retention - 1` 个月，**不是** `run_date`。
差别是承重的：锚在窗口上，`lookback_days` 配多大都不可能删掉本轮要读的月份；
锚在今天就要额外记住「窗口有没有跨月」，多一处能静默删错的地方。

`%Y%m` 的字典序等于时间序，所以比一次字符串就够 —— 路径**只被拼，不被拆**，
和 `layout.rs` 其余部分同一个规矩。

门槛按「必须长得像月目录」设（是目录 · 名字恰好 6 位数字 · 严格早于保留起点）：
这是全仓唯一一处递归删生产数据的地方，松一位就意味着 `raw_root` 底下手工放的
任何东西都会在某天被静默删掉。删不掉只记 warn —— 磁盘继续涨不是数据错，
不该掀翻这一轮。

**磁盘上界从「无」变成约 2 个月**（1000 群约 36 GB）。

⚠️ 一个已认领的代价：`lookback_days` 事后调大到跨过保留期时，那些月文件已经删了，
`have = 0` 会**整月重下**。正确但很慢。调大 lookback 之前先看一眼保留期。

### 为什么不是「只留窗口那几天的字节」

设计中途认真算过：记下每轮的字节水位、只 Range 取「水位 → 现在」，
读完即弃。**逻辑上成立**，靠的正是本文档已经在用的那条恒等式
（追加时刻 ≥ 消息时刻，见 `ndjson_last_append_time >= ?` 那道筛选）。
一个容易错的细节记在这里：起点必须取 **T-N-1** 那轮的水位而不是 T-N 那轮的 ——
T-N 当天 00:00 到跑批时刻之间到达的消息，在窗口里却在 T-N 那个水位**之前**。

**不做，因为账算不过来：**

| | 磁盘（1000 群） | 新状态 | `getsize == position` |
|---|---|---|---|
| 无清理（此前） | 216 GB/年，无上界 | — | ✅ |
| **保留 2 个月**（本决策） | ~36 GB 滚动 | **零** | ✅ |
| 水位 + 只留 6 天 | ~3.6 GB | 水位表，或文件名带偏移 | ❌ 要重新发明 |

多买 32 GB，要付一套状态机，外加丢掉规则 5 那道端到端校验 ——
而那道校验是 CDN 陈旧缓存唯一的防线。32 GB 在服务器上不是钱。

（若哪天真要做：保住校验的最省写法是文件名带起始偏移
`<roomId>.<offset>.ndjson`，`have = offset + getsize`。代价是 `layout.rs` 多一处
「拆布局」—— 今天布局只被拼、从不被拆。）

### 9. 下钻超出保留期时回 OSS 取（**方向已定，随 webUI 落地**）

保留期把「本地有没有那几行原文」变成了有限的：超出保留期的事件，
`read_by_ids` 会显式报「取不到这些 msg_id」（**不静默少给**，那条规矩不变）。
定的方向是 **DuckDB 直读 OSS 兜底**：本地有就读本地（单群单月 ≈ 70 ms、字节校验过），
本地没有才走 URL，慢几秒可以接受。

**今天不写**，判据是「没有调用方就没有接缝」：`read_by_ids` 目前零调用方
（webUI 那条旁路还没搬），而它的签名完全由 webUI 的形状决定 ——
`ndjson_object_key` 今天只住 `mirror/index.rs`，兜底要用它就得回答
「多一个参数，还是让 `ingest` 也认识索引表」，那个答案在 webUI 存在之前只能靠猜。

落地时的三件事，先记在这里：

1. **`scan()` 的 `files: &[(String, PathBuf)]` 改成 `&[(String, String)]`（月份, 位置）** ——
   DuckDB 的 `filename` 列原样还回你传进去的字符串，本地路径和 URL 走同一条路，
   月份守卫、去重、五道守卫一行不用改。这是全部的结构改动。
2. **兜底路径没有字节数校验，而 CDN 那 30 天缓存还在**（本文档结尾那个未解问题）。
   老事件反而安全（字节早就稳定），**越近的事件越可能撞上陈旧副本** —— 跟直觉相反。
   这也是保留期不该配得太短的真正理由：本地那份是被校验过的，OSS 那份不是。
3. **`INSTALL httpfs` 首次要联网**，落在 `~/.duckdb/extensions`。跑批机器可能不给出网。
